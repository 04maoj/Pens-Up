...
public class Alphabate_manager : MonoBehaviour
{
    ...
    private void ResetStrokeNum(int num)
    {
        stroke_number = num;
    ...
    /// <summary>
    /// Return Possible Strokes given hit box
    /// </summary>
    /// <returns></returns>
    public HashSet<int> F_GetsPossibleStrokWithHitBoxId(int hitBoxId)
    {
        HashSet<int> possBoxes = new HashSet<int>();
        for (int i = 0; i < stroke_number; i++)
        {
            foreach(int box in strokes_and_hit_box[i])
            {
                if (box == hitBoxId)
                {
                    possBoxes.Add(i);
